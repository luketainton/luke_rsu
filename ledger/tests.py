...
class WorkspaceIsolationTests(TestCase):
    ...
    def test_dashboard_shows_compact_summary_and_record_pages_are_isolated(self):
        ...
        self.assertContains(response, 'href="https://git.tainton.uk/repos/rsu/releases/tag/v0.3.0"')
        self.assertContains(response, ">v0.3.0</a>")
        self.assertNotContains(response, "M4 19V5m0 14h16")
    ...
    def test_section_104_does_not_match_acquisition_after_30_days(self):
        workspace = self.bob.workspace_memberships.get().workspace
        broker = Broker.objects.create(workspace=workspace, name="Charles Schwab")
        security = Security.objects.create(workspace=workspace, name="Microsoft", ticker="MSFT")
        grant = Grant.objects.create(
        ...
        report = section_104_report(security, [grant], [vest, future_vest], [sale])
        self.assertEqual(report.disposals[0].matches[0].kind, "Section 104 pool")
        self.assertEqual(report.pool_units, Decimal(3))
    ...
    def test_editor_can_edit_and_delete_an_hmrc_rate(self):
        ...
        self.client.force_login(self.bob)
        response = self.client.get(reverse("delete_rate", args=[rate.id]))
        self.assertContains(response, "Delete exchange rate?")
        self.assertContains(response, "Delete exchange rate")
        self.assertNotContains(response, "Delete HMRC rate")
        response = self.client.post(
